...
vault q orphans / type / tag / stats
```
...
원본 데이터가 없는 상태에서 그래프를 만드는 프로젝트와 이 프로젝트는 출발점이 다르다.

```
일반적인 경우:  데이터 없음 → 파이프라인이 그래프를 생성      (쓰기 중심)
이 프로젝트:     문서 6,452개가 이미 있음 → 그래프를 추출      (읽기 중심)
```

| | 그래프의 지위 | 틀렸을 때 |
|---|---|---|
| 일반적인 경우 | **원본** — 유일한 저장소 | 데이터 손상. 복구 필요 |
| **이 프로젝트** | **파생물** — md가 원본 | **다시 빌드하면 끝** |
...
`part_of`(디렉터리 계층)는 frontmatter에 안 쓴다. **경로가 이미 알고 있으므로 빌드 시점에 유도한다.** 저장하면 반드시 어긋난다.
...
"X를 이해하려면 뭘 먼저 봐야 하나"는 `builds_on`을 **여러 단계 따라가는** 질문이다.
...
노드·엣지·태그·kind별 개수를 비교한다. 값이 다르면 어느 Phase에서 달라졌는지 역추적하고, 모든 차이의 원인을 설명해야 한다.
...
**세 축은 정확히 일치했고, 값이 달랐던 나머지 세 항목은 새 구현이 맞았다.**
...
- 태그 — 답안지는 **2026-08-11 고정 스냅샷**이라 본문에서 읽는다. 8월 16일 일원화로 본문 태그는 0이 됐다. **현재 vault에서는 답안지의 태그 집계가 계속 0으로 나온다**

마지막 줄 때문에 「전부 일치」라는 기준을 바꿨다. **vault뿐 아니라 비교 기준인 답안지도 특정 시점에 고정돼 있다.**
