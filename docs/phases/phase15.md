...
출처**를 가리키는 의도된 값이고, 해석되지 않는 것이 정상이다. `v:supersedes_raw`가
이미 같은 자리에 있다 — 「the NORMAL case, not a fault」.

> **미결.** 지금은 깨진 링크와 같은 술어에 앉는다. 사건 문서 소급 작성이 진행되면
> 개수가 늘 것이고, 그때 별도 값이 필요한지 다시 본다. 지금 어휘를 만들면 용례 2건짜리
> 술어가 하나 생긴다.
