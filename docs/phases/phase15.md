...
`NAMED = ("builds_on", "supersedes")`가 아직 그대로다. **`derived_from` 69건을
비롯한 의미 관계 84건이 그래프에 안 들어가 있다.** 앵커 해석(`[[문서#인사이트 20]]`이
대상 문서의 항목 제목과 맞는지)이 해석기 신호를 바꾸므로 다음 라운드로 뗀다.
**gold set 재현은 그것과 함께 한다.**
