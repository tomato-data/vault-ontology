...
```
기본        문서                     3,984개 (98.3%)
하위 단위    제목(heading)            67개 문서 · 항목 324개
안 만든다    블록 ID · 문장 · 줄 번호
```

「최소 단위를 제목으로 바꾼다」가 아니다. **필요한 데서 제목까지 내려간다.**

판별 기준 — **번호 붙은 항목 제목이 하나라도 있으면 그 제목이 주소를 갖는다.**

> **2026-08-31 개정.** 여기 원래 「3개 이상」이라고 적혀 있었다. 그 숫자는 Phase 13이
> **「이 vault에 문서보다 작은 단위가 필요한가」를 재려고 쓴 개체수 필터**였는데
> (「49개 문서 320항목」), Phase 15 Step 1이 그것을 발급 규칙으로 굳혔다. 왜 3인지는
> 어디에도 적혀 있지 않았다. Step 4 실측이 세 가지를 보였다.
>
> ```
> 문턱은 항목이 아니라 문서를 잘랐다       항목 292 → 324.  11%
> principle 10개가 가려져 있었다          「사례 N — 사건 (2026-04-29)」— C04가 찾던 근거
> 문턱이 취약성을 만들었다                항목 3개짜리 16문서는 한 줄 지우면 섹션 3개가 사라졌다
> ```
>
> 낮춰도 안전한 이유 — **한 문서에 같은 항목 제목이 두 번 나오는 경우가 0건**이라
> IRI가 안 겹치고, **292가 324의 부분집합**이라 기존 주소가 하나도 안 움직인다.
