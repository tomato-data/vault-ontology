...
- [ ] `vault-ontology.ttl`이 데이터와 **별도 파일**로 존재한다 (스키마와 데이터의 분리)
- [ ] 표준 어휘(SKOS·dcterms)를 **실제로 재사용**했다 — 전부 자기 술어면 실패
- [ ] `type` 계층 질문에 **답을 냈다** — 있다면 근거 질의, 없다면 왜 없는지
- [ ] 태그 계층이 `skos:broader`로 표현됐다
- [ ] 모든 술어에 "어떤 질의에 답하나"가 적혀 있다 — 못 적으면 그 술어는 지운다
