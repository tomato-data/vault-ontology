...
vault 문서의 맨 앞 `---` 블록에는 `type`, `summary`, `builds_on`, `created`가 들어 있다. **그래프의 노드 속성과 명시적 관계가 전부 여기서 나온다.** 이걸 못 읽으면 뒤의 모든 Phase가 없다.
...
첫 Phase다. 대신 **이 함수들이 뒤에서 어떻게 쓰이는지**를 알고 시작한다.
