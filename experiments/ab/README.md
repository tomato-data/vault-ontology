...
| 코퍼스 | 문서 3,862 → 청크 43,715 (캡 없음) · 2026-08-25 집 M5 Pro 실측 |
| `max_seq_length` | 1024 — 최장 청크가 905 토큰이라 **무손실** |
...
> **768 → 1024 (2026-08-25).** 회사 세션이 768 을 고른 근거는 「최장 청크 767 토큰」이었다.
> vault 가 자라 청크 43,715 개 중 **45 개(0.10%)가 768 을 넘었고 최장이 905** 가 됐다.
> 768 그대로면 그 45 개의 꼬리가 소리 없이 잘린다. **어느 팔도 돌기 전에** 올렸으므로
> 네 팔은 여전히 같은 설정을 쓴다. 배치 패딩은 배치 내 최장 기준이라 비용은 그 45 개가
> 든 배치뿐이다.
