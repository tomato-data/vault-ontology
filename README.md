...
vault의 취지 자체가 **개발하면서 얻은 지식의 데이터베이스**였다. 문서 4,204개가 위키링크 11,624개로 이미 얽혀 있다. 사람 눈에는 분명 그래프인데, **기계가 읽을 수 있는 구조가 없다.** 처음에는 온톨로지가 정확히 그 자리에 들어맞는 도구라고 생각했다. 이 프로젝트는 그 가설을 실제 vault로 검증한 기록이다.
